...
class VersionUnitManagerTest() extends TestKit(ActorSystem("VersionUnitManagerTest")) with ImplicitSender
  ...
  "A VersionUnitManager" - {
    ...
    var versionUnitManager: ActorRef = null
    ...
    "should get the FileSources and instantiate two child workers" in {
      ...
      versionUnitManager = system.actorOf(VersionUnitManager.props(projectId, pathToProjects,  versionList,
    ...
    "should have the two versions in the remaining list" in {
      ...
      versionUnitManager ! GetRemainingList()
      expectMsg(3.seconds, expectedList)
    }

    val childMock3 = TestProbe()
    val VRU1 = ModelMock.versionResearchUnitMock(versionName = versionModel1.versionName)
    "should receive a VersionResearchUnit and trigger another version extraction from the remaining list" in {
    ...
    "should have a done list with the VRU1" in {
      val expectedList = List(VRU1)
      versionUnitManager ! GetDoneList()
      expectMsg(3.seconds, expectedList)
    }
    ...
    "should still have a done list with only the VRU1" in {
      val expectedList = List(VRU1)
      versionUnitManager ! GetDoneList()
      expectMsg(3.seconds, expectedList)
    }
    ...
    val VRU3 = ModelMock.versionResearchUnitMock(versionName = versionModel3.versionName)
    "should receive a VersionResearchUnit and update its internal state correctly" in {
      versionUnitManager ! VRU3
      Thread.sleep(200)
      ...
      val expectedDoneList = List(VRU1, VRU3)
      versionUnitManager ! GetDoneList()
    ...
    val VRU4 = ModelMock.versionResearchUnitMock(versionName = versionModel4.versionName)
    "should receive the last VersionResearchUnit and write it to csv format" in {
      (mockSourceApi.exportToCsv _).expects(List(VRU1, VRU3, VRU4), "./target/versionUnits.csv")
      versionUnitManager ! VRU4
      Thread.sleep(200)
    }
  }
}